import {Collection, Data, DirectoryObject, FileObject, FS, Tantrum, Util} from "@gesslar/toolkit"
...
export default class Configuration {
        ...
        if(key === "include" || key === "exclude") {
          if(Data.isType(value, "array")) {
            ...
            if(settled.some(r => r.status === "rejected"))
              throw Tantrum.new(
                `Loading files from ${key}`,
                settled.filter(r => r.status === "rejected").map(r => r.error)
              )

            value = settled.map(f => f.value)
   ...
   * @returns {Promise<Array<object>>} All options from all sources.
   */
   ...
   *
   * @param {Array<object>} allOptions - All options from all sources.
