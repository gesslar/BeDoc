export const actions = [
  {
    ...
    tags: {
      ...
      singletons: ["name", "return", "example", "meta"],
    ...
    processLine({line, func, file, position}) {
      ...
      if(!func)
        return {
          status: "error",
          message: msg("No function context", "processLine", file, position, line),
        }
      ...
      if(tagMatches) {
        ...
        if(!isValid)
          return {
            status: "error",
            message: msg(`Invalid tag \`${tag}\``, "processLine", file, position, line),
          }
        ...
        if(singleton) {
          if(func[tag])
            return {
              status: "error",
              message: msg(`Singleton tag already exists: ${tag}`, "processLine", file, position, line),
            }
          func[tag] = null
        } else {
          func[tag] = func[tag] || []
        ...
        if(tag === "return") {
          ...
          if(tagContentMatches) {
            ...
            if(!type)
              return {
                status: "error",
                message: msg(`Missing return type: ${tag}`, "processLine", file, position, line),
              }
            if(!content) {
            ...
            return {
              status: "error",
              message: msg("Failed to parse return tag", "processLine", file, position, line),
            }
        } else {
          ...
          if(tagContentMatches) {
            ...
            if(!type)
              return {
                status: "error",
                message: msg("Missing tag type", "processLine", file, position, line),
              }
            if(!name)
              return {
                status: "error",
                message: msg("Missing tag name", "processLine", file, position, line),
              }
            ...
            if(this.tags.singletons.includes(tag)) {
              ...
              return {
                status: "error",
                message: msg("Failed to parse tag", "processLine", file, position, line),
              }
            }
          }
        }
