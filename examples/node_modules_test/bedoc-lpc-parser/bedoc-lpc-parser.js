...
export default class LpcParser {
  ...
  #gimme = ob => Object.fromEntries(Object.entries(ob).filter(([_, v]) => v != null))
  ...
  #extractTags = async ctx => {
    ...
    // Might as well get this setup. If you aren't aware, arrays are
    // addressed by... well, address. Referenced by reference... things.
    // SO, we can just push to it and it'll be awesome. and you don't
    // have to worry your pretty little head about it. All right? Let's go.
    ...
    while(lines.length > 0) {
      ...
      if(!(tag in extractedTags))
        extractedTags[tag] = []

      extractedTags[tag].push(groups)
    }
