...
export default class LPCParser {
  ...
  #findFrom(lines,pattern,start) {
    return lines.findIndex((line,i) => i >= start && pattern.test(line))
  }
  ...
  async #extractBlocks(value) {
    ...
    while(lines.length) {
      ...
      // Find the function
      const idIndex = this.#findFrom(lines,funcPattern,0)

      // Moar peekahoohoo
      const nextBlockIndex = this.#findFrom(lines,blockStartPattern,-1)
      ...
      if(idIndex > -1) {
        ...
        if(nextBlockIndex !== -1 && idIndex > nextBlockIndex) {
          ...
          // Whew! Safe.
          const func = funcPattern.exec(lines[idIndex])
