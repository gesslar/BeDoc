/**
 * @file Wikitext Printer - A formatter for converting structured documentation
...
export default class WikitextPrinter {
  ...
  static meta = Object.freeze({
    ...
    format: "wikitext",
    extension: "txt",
    terms: "ref://./bedoc-wikitext-formatter.yaml"
  })
  ...
  #formatFunction = ctx => {
    ...
    if(ctx.signature) {
      ...
      const signature = [
        ...
        sig.type ? `''${sig.type}''` : "",
        sig.name ? `'''${sig.name}'''` : "",
        sig.parameters?.length ? `(${sig.parameters.join(", ")})` : "()"
      ].filter(Boolean)
