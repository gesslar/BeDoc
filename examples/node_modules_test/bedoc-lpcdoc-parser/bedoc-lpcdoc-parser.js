...
import {Collection,Data,Term,Util} from "@gesslar/toolkit"

const {WHILE,SPLIT} = ACTIVITY
 ...
 * @class
 */
export default class LPCDocParser {
  /**
  ...
  static meta = Object.freeze({
    kind: "parse",
    input: "lpc",
    terms: "ref://./bedoc-lpcdoc-parser.yaml",
    name: "LPCDocParser"
  })
  ...
  setup = ab => ab
    ...
    // Finalize results
    .do("Finalise", this.#finally)

  #findFrom(lines,pattern,start) {
    const result = lines.findIndex((line,i) => i >= start && pattern.test(line))
  ...
  async #extractBlocks(value) {
    // await timeout(1_000 + ++this.#it*300)
    const {content} = value
    const result = []
    const lines = Data.appendString(content, "\n").split("\n")
    const blockStartPattern = this.#regexes.get("block-start")
    const blockStopPattern = this.#regexes.get("block-stop")
    const funcPattern = this.#regexes.get("function")

    while(lines.length) {
      const block = {}

      // Find the block start index
      const startIndex = this.#findFrom(lines,blockStartPattern,0)

      if(startIndex < 0)
        break

      // The peekahead
      const peekStartIndex = this.#findFrom(lines,blockStartPattern,startIndex+1)

      // Find the block end index
      const endIndex = this.#findFrom(lines,blockStopPattern,startIndex)

      // Hmmmmm! I guess we done here?
      if(endIndex == -1)
        break

      if(peekStartIndex > -1 && endIndex >= peekStartIndex)
        break
      ...
      // Find the function
      const functionIndex = this.#findFrom(lines,funcPattern,0)

      // Moar peekahoohoo
      // const nextBlockIndex = this.#findFrom(lines,blockStartPattern,nextBlockIndex+1)

      if(functionIndex > -1) {
        // they can't be equal, they're different patterns
        if(peekStartIndex !== -1 && functionIndex > peekStartIndex) {
          ...
          lines.splice(0, peekStartIndex)

        } else {
          block.lines = lines.slice(startIndex+1,endIndex)
          // Whew! Safe.
          const func = funcPattern.exec(lines[functionIndex])
          ...
          // Slurp! Slurp!
          lines.splice(0, peekStartIndex > -1 ? peekStartIndex : lines.length)
        }
      }

      if(block.lines && block.function)
        result.push(block)
    }
